...
function gotoReader(id, serialNumber,title) {
    wx.redirectTo({
        url: `../reader/reader?id=${id}&serialNumber=${serialNumber}&title=${title}`
    });
}
...
Page({
    ...
    toReader: function (e) {
        ...
        gotoReader(that.data.id, serialNumber,e.currentTarget.dataset.title);
    },
    ...
    initChapters: function (id, serialNumber, loadPageIndex) {
        var that = this;
        let url = app.globalData.contentUrl + `?nid=${id}&serialNumber=${serialNumber}`;
        wx.showLoading({
            title: '加载中',
            mark: true
        });
        ...
        https.get(url, function (res) {
            try {
                ...
                var nextItemId = res.nextItemId;
                var preItemId = res.preItemId;
                var id = res.id;
    ...
    initCatelog: function (id) {
        var that = this;
        let url = app.globalData.catalogUrl + `?nid=${id}`
        wx.showLoading({
            title: '加载中',
            mark: true
        });
